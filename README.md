...
- Fotos de loja/produto: bucket `media` no Supabase Storage, leitura pública (as imagens aparecem no app pra qualquer visitante) e escrita restrita — cada lojista só grava/atualiza/apaga dentro da própria pasta (`<user_id>/...`), validado por policy de RLS no próprio Storage. Upload é limitado a 5 MB e aos tipos `image/jpeg`, `image/png`, `image/webp`, `image/gif` (validado no bucket e também no client antes do envio).
