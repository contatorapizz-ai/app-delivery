...
- **Localização**: cada usuário logado tem uma cidade salva no próprio perfil (`profiles.city`), editável em `/configuracoes` ou direto na barra do topo — funciona em qualquer aparelho que a pessoa entrar. Sem login, a cidade fica só no aparelho (localStorage).
